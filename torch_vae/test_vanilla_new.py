...
if __name__ == '__main__':
    ...
    logdirpath = '/workspace/torch_vae/logs/VanillaVAE_new/version_0'
    checkpoint_path = os.path.join(logdirpath, 'checkpoints', 'last.ckpt')
    config_path = os.path.join(logdirpath, 'configs', 'config.yml')
    with open(config_path, 'r') as config_file:
        config = yaml.safe_load(config_file)

    # Load the trained LightningModule from the checkpoint
    vae_model = VanillaVAE(**config['vae_config'])
    trained_model = VAELightning.load_from_checkpoint(
        checkpoint_path=checkpoint_path,
        vae_model=vae_model,
        params=config['lightning_config']
    )
    ...
    mnist_datamodule = MNISTDataModule(data_dir=config['data_config']['data_dirpath'])
...
if True:
    ...
    char_images_list, char_labels = load_specific_labels(engchar_datamodule, ['K'])
